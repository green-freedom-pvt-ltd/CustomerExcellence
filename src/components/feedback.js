...
export default class Feedback extends Component {
  ...
  handleSelect(eventKey) {
    ...
    else if (this.state.activePage === eventKey + 1) {
      this.fetchResults(this.state.prevPage);
    }
    else {
      if(this.state.fetchUrl){
        //This is for main feedback page
        if(!this.state.fetchUrl.includes('user_id')){
         // console.log("Came in IF block",this.state.fetchUrl)
          
          var paginateUrl = this.state.fetchUrl +'?page='+ eventKey
          this.fetchResults(paginateUrl);
        }
        else{
          //This is for particular user's feedback
          var paginateUrl = this.state.fetchUrl +'&page='+ eventKey
          this.fetchResults(paginateUrl);
        }
